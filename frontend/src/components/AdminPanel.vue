...
import { api, type CurrentUser, type SystemSettings, type SystemSettingsUpdate } from "../api";
  ...
  <section class="content admin-content">
    <header class="content-header">
      <div>
        ...
        <p class="subhead">{{ mode === "users" ? "创建、停用或重置成员账户。" : "这些配置即时用于模型、检索和文件处理；部署和启动参数仍保留在环境变量中。" }}</p>
      </div>
    </header>
    <div v-if="error" class="error-banner">{{ error }}</div>
    <p v-if="success" class="success-banner">{{ success }}</p>
    <div v-if="busy && !settings && !users.length" class="state-line"><LoaderCircle class="spin" :size="18" />正在加载</div>
    ...
    <form v-else-if="settings" class="admin-settings" @submit.prevent="saveSettings">
      <section class="admin-settings-section">
        <h2>大语言模型</h2>
        <label>服务地址<input v-model="config.llm_base_url" placeholder="https://api.example.com/v1" /></label>
        <label>模型名称<input v-model="config.llm_model" /></label>
        <label>API 密钥 <small>{{ settings.llm_api_key_configured ? "已配置" : "未配置" }}</small><input v-model="config.llm_api_key" type="password" placeholder="留空保持不变" /></label>
        <label>温度<input v-model.number="config.llm_temperature" type="number" min="0" max="2" step="0.1" /></label>
      </section>
      <section class="admin-settings-section">
